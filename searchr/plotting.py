...
def test_plot(df):
    ...
    #sns.regplot(data=df, x="Age", y="Beta_percent") # if Lowess = FALSE
    ...
    plt.fill_between(x_pred, y1=upper, y2=lower, alpha=0.1, color='tab:blue') # shaded conf interval
...
def sex_plot(df):
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    sns.scatterplot(x=df["Age"], y=df["Beta_percent"], hue=df["Sex"], s=55) # s = size of points. Increased to match point sizes in test_plot()
    plt.xlabel("Age (PCW)", fontsize=15)
    plt.ylabel("DNA methylation (%)", fontsize=15)
    FigureCanvas(fig)
    img = io.BytesIO()
...
def test_plot1(df):
    ...
    xseq = np.linspace(min(x), max(x), num=100) # seq of 100 values between smallest to largest age
